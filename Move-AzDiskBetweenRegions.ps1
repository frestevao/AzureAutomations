...
$status = Get-AzStorageBlobCopyState -Blob $destinationVHDFileName -Container $storageContainerName -Context $destinationContext
$sizeGB = [math]::Round(($status.TotalBytes)/1GB)
$copyGB = [math]::Round(($status.BytesCopied)/1GB)
$percent = [math]::Round(($copyGB/$sizeGB)*100)
$timeTaken = [math]::Round(((Get-Date) - $startTime).TotalMinutes)
Write-Host "Status em:" $(Get-Date -Format HH:mm) -ForegroundColor Yellow
Write-Host "Status:" $status.Status -ForegroundColor Yellow
Write-Host "Tamanho:" $sizeGB -ForegroundColor Yellow
Write-Host "Copiado:" $copyGB -ForegroundColor Yellow
Write-Host "Percent:" $percent "%" -ForegroundColor Yellow
Write-Host "Tempo em mins:" $timeTaken -ForegroundColor Yellow